...
A complete, self contained hardware and firmware project for a custom addressable LED lighting system: a custom designed **ESP32-S3 controller motherboard**, three **daisy chainable WS2812B light stick PCBs**, and a tailored build of **[WLED](https://github.com/wled/WLED) v16.0.1** configured for the exact hardware. This repository contains everything needed to reproduce the system end to end: Gerber and drill files, pick and place data, bills of materials, 3D STEP models, the full patched firmware source tree, and a prebuilt single file flash image.
...
- **Power path:** `GPIO2` controls an **N-channel + P-channel MOSFET high-side switch** (active-high) so the entire LED rail can be de energized in software, giving true zero standby draw on the strip, not just black pixels.
...
- **Single merged flashable image**: bootloader, partition table, and application combined into one `.bin` written at offset `0x0`. No multiple offset flashing.
...
- **Built on a stable upstream release** (`v16.0.1`), with the full source tree preserved in the repository for reproducible builds and future upstream syncs.
...
| n/a | 8× resistors (2×1 kΩ, 2×5.1 kΩ, 3×10 kΩ, 1×330 Ω) | 0805 | Gate drive, CC1/CC2 termination, pull-ups, charge programming |
| n/a | 2× 1 µF capacitors | 0805 | LDO input/output decoupling |
| J2 | 1×4 pin header (DNP) | 2.54 mm | Auxiliary/debug header |
| n/a | 5× test points (`5V`, `B+`, `B−`, `DIN`, `GND`) | 3.0 mm pads | Bringup and battery/strip wiring |
...
| LED count | n/a | `PIXEL_COUNTS=21` |
| Flash | n/a | 16 MB, DIO, 40 MHz |

---

## Light Sticks

Three variants of the same design language: a 5 V WS2812B chain with 0.1 µF decoupling per-LED and pad-based chaining:
